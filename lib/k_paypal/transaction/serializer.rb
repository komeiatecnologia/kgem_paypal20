    ...
    class Serializer
      ...
      def serialize_set_express_checkout(data)
        data[:set_express_checkout] = {
          ...
          :ack => hash[:ACK],
          :build => hash[:BUILD],
          :time_stamp => covert_time_parse(hash[:TIMESTAMP].to_s)
        }
      end
      ...
      def serialize_get_express_checkout(data)
        data[:get_express_checkout] = {
          ...
          :amt => convert_string_to_bigdecimal(hash[:AMT].to_s),
          :item_amt => convert_string_to_bigdecimal(hash[:ITEMAMT].to_s),
          :shipping_amt => convert_string_to_bigdecimal(hash[:SHIPPINGAMT].to_s),
          :handling_amt => convert_string_to_bigdecimal(hash[:HANDLINGAMT].to_s),
          :tax_amt => convert_string_to_bigdecimal(hash[:TAXAMT].to_s),
          :inv_num => hash[:INVNUM],
          :insurance_amt => convert_string_to_bigdecimal(hash[:INSURANCEAMT].to_s),
          :ship_disc_amt => convert_string_to_bigdecimal(hash[:SHIPDISCAMT].to_s),
          :payment_request_currency_code => hash[:PAYMENTREQUEST_0_CURRENCYCODE],
          :payment_request_amt =>  convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_AMT].to_s),
          :payment_request_item_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_ITEMAMT].to_s),
          :payment_request_shipping_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_SHIPPINGAMT].to_s),
          :payment_request_handling_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_HANDLINGAMT].to_s),
          :payment_request_tax_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_TAXAMT].to_s),
          :payment_request_inv_num => hash[:PAYMENTREQUEST_0_INVNUM],
          :payment_request_insurance_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_INSURANCEAMT].to_s),
          :payment_request_ship_disc_amt => convert_string_to_bigdecimal(hash[:PAYMENTREQUEST_0_SHIPDISCAMT].to_s),
      ...
      def serialize_do_express_checkout(data)
        data[:do_express_checkout] = {
          ...
          :payment_info_order_time => covert_time_parse(hash[:PAYMENTINFO_0_ORDERTIME].to_s),
          :payment_info_amt => convert_string_to_bigdecimal(hash[:PAYMENTINFO_0_AMT].to_s),
          :payment_info_fee_amt => convert_string_to_bigdecimal(hash[:PAYMENTINFO_0_FEEAMT].to_s),
          :payment_info_settle_amt => convert_string_to_bigdecimal(hash[:PAYMENTINFO_0_SETTLEAMT].to_s),
          :payment_info_tax_amt => convert_string_to_bigdecimal(hash[:PAYMENTINFO_0_TAXAMT].to_s),
      ...
      def serialize_get_transaction(data)
        data[:get_transaction] = {
          ...
          :order_time => covert_time_parse(hash[:ORDERTIME].to_s),
          :amt => convert_string_to_bigdecimal(hash[:AMT].to_s),
          :currency_code => hash[:CURRENCYCODE],
      ...
      def convert_string_to_bigdecimal(value)
        begin
          BigDecimal(value)
        rescue ArgumentError
          BigDecimal(0)
